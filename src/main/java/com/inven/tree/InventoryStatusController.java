...
public class InventoryStatusController {
    ...
    public ResponseEntity<List<InventoryStatus>> getInventoryStatus(@PathVariable String corpIdx) {
        try {
            ...
            List<InventoryStatus> inventoryStatusList = products.stream()
                .filter(product -> product != null)  // null 제외
                .map(product -> {
                    ...
                    Warehouses warehouse = warehouses.stream()
                        .filter(w -> w.getWhIdx().equals(product.getWhIdx()))
                        .findFirst()
                        .orElse(null);
                    Shelves shelf = shelves.stream()
                        .filter(s -> s.getShelfIdx().equals(product.getShelfIdx()))
                        .findFirst()
                        .orElse(null);
