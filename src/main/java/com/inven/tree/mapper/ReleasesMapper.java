...
public interface ReleasesMapper {
    ...
    // 사용자의 회사 코드와 날짜로 출고 정보 불러오는 메소드
    List<Releases> selectByCorpIdxAndDate(@Param("corpIdx") String corpIdx, @Param("year") int year,
                                          @Param("month") int month, @Param("prodIdx") Integer prodIdx);

    // 입출고 내역 중 출고 내역 필터링 및 페이징 메소드
