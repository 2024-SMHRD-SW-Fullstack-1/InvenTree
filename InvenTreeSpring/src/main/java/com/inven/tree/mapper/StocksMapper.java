...
public interface StocksMapper {
	...
	// 입고 등록(저장)
	void insertStock(@Param("corpIdx") String corpIdx, @Param("prodIdx") int prodIdx, @Param("stockCnt") int stockCnt,
			@Param("stockedAt") Timestamp stockedAt);
	...
	// 오늘 날짜의 입고 데이터를 가져오는 메소드
    List<Stocks> findTodayStocksByCorpIdx(@Param("corpIdx") String corpIdx);

	// 회사 코드로 상위 변동 재고를 가져오는 메소드
	List<Stocks> findTopStockChanges(@Param("corpIdx") String corpIdx);
	
	// 회사 코드로 재고를 가져오는 메소드
    List<Stocks> findStocksByCorpIdx(String corpIdx);
